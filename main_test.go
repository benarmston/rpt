...
func TestDelay(t *testing.T) {
	tests := []struct {
		...
		{
			"every - leading edge; invocation duration less than --every duration",
			...
			[]string{"--every", "2s", "3", "bash", "--", "-c", "sleep 1; date +%s"},
			2,
			0,
		},
		{
			"Run date every 2s - leading edge; invocation exceeds --every duration",
			...
			[]string{"--every", "1s", "3", "bash", "--", "-c", "sleep 2; date +%s"},
			2,
			0,
		},
	}
